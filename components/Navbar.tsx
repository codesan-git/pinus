...
const Navbar: FC<NavbarProps> = () => {
    ...
    const navbarOptions = [
        ...
        { name: "Notifications", href: "/notifications", icon: BellIcon, current: `/${segment}` === "/notifications" ? true : false },
        { name: "Messages", href: "/messages", icon: EnvelopeOpenIcon, current: `/${segment}` === "/messages" ? true : false },
    ]
                            ...
                            {navbarOptions.map((option) => (
                                <li key={option.name} className=''>
                                    <Link href={option.href}>
                                        <div className={classNames(option.current ? "text-black font-bold" : "text-gray-600 hover:text-black", "group flex p-2 rounded-full leading-6 space-x-4 text-sm font-medium overflow-hidden transition-all")}>
